"""
Restart-safety boundary, reference-free diagnostic, and grid-refinement control.
Author: Dharmapala R.D. (214050V)

Builds on the low-viscosity stress test (viscosity_reanchor_stress[_ext].py).
NO new mechanism; three evidence items:

  A) GRID-REFINEMENT CONTROL (closes the under-resolution confound).
     The stress test scores against a same-scheme, same-grid dt-refined
     reference, so spatial under-resolution would be invisible to it.  Here the
     verified scheme is re-run at N = 1024 (and 2048 where affordable) from
     t = 0 and compared, restricted to the shared 512 grid, against the
     N = 512 solution.  If they agree to well under the 1% target, the 512-grid
     verified solution is physically converged, not merely self-consistent --
     "restart fidelity matters" survives the "everything under-resolves down
     there" question.

  B) SAFETY BOUNDARY.  The careless (non-de-aliased) restart's failure is
     mapped against the cell Reynolds number at the handoff,
         Re_cell = max|u(t_s)| * dx / nu,
     giving a predicted boundary (Re_cell where the careless tail crosses the
     1% and 5% targets) instead of a list of failing viscosities.

  C) REFERENCE-FREE DIAGNOSTIC.  The high-wavenumber energy fraction of the
     handoff state (top octave of the resolved band, computable from the ML
     state alone -- no ground truth) is evaluated as a predictor of approximate-
     restart failure.  This is the same spectral-content idea as the Module 2
     robustness work, now as a restart-safety check.

Honesty rules (pre-registered): if the refinement control shows the 512 grid
is NOT converged at the lowest viscosity, that is reported and the stress-test
claim stays scoped to fixed-grid restart fidelity.  The boundary is reported
with its threshold sensitivity (1% and 5% crossings both given).

Outputs: results/module2/figures/restart_safety_boundary.json / .png
         (+ headline figure restart_fidelity_headline.png)
"""
