"""
PRELIMINARY Day-2 gate + hybrid sweep, offline from results/eval/predictions.npz.
This is a preliminary experiment, NOT a final thesis result:
  * n = 10 unseen initial conditions
  * raw FNO-state handoff (no filtering)
  * numpy restart wrapper (team-scheme port; exact equivalence not yet verified)
  * real viscosity nu = 1/(100 pi)
Scope: the switch time is SUPPLIED to this experiment (a teammate's trust signal
will later decide it). We measure what happens for each supplied switch time.

Three trajectories per (IC, switch time):
  UB : restart spectral from TRUE Cole-Hopf state at t_s (upper bound / gate)
...
OUT_DIR = os.path.join(ROOT, "results", "module2")
...
def main():
    ...
    print("*** PRELIMINARY *** %d held-out ICs | nu=%.6f | raw handoff | numpy wrapper\n" % (n_ic, NU))
    print("%4s %4s %9s %9s %9s %9s %7s %6s" %
          ("t_s","idx","e_s(FNO)","UB_tail","FNO_tail","H_tail","B_int","ICs+"))
    print("-"*62)
    gate_ok = True; rows = []
    ...
    payload = {
        "status": "PRELIMINARY -- not final thesis results",
        "n_ic": n_ic, "nu": float(NU), "handoff": "raw FNO state (no filtering)",
        "reference": "Cole-Hopf (u_true_eval)",
        "continuation_solver": "numpy restart wrapper (team-scheme port; equivalence not yet verified)",
        "switch_time_source": "externally supplied (not decided by this module)",
        "gate_pass": bool(gate_ok),
        "results": rows,
    }
    out = os.path.join(OUT_DIR, "day2_sweep_results.json")
