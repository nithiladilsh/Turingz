...
OUT = os.path.join(_ROOT, "results", "deeponet_relL2")
...
class DeepONetDDE(AbstractSolver):
    ...
    def fit(self, dataset):
        ...
        self.model.compile("adam", lr=self.lr, loss=rel_l2_loss, metrics=["l2 relative error"])
...
def main(mode="full"):
    ...
    if mode == "smoke":
        ...
        iters, seeds = ITERS, SEEDS

    ds = {"u": U, "ICs": ICs, "x": x, "t": t, "t_train_end": te, "T": Tmax,
          "train_idx": train_idx, "val_idx": val_idx}

    runs, best, best_solver = [], None, None
    for sd in seeds:
    ...
    info = {"model": "DeepONet(DeepXDE, relL2)", "library": "deepxde", "loss": "relative_l2",
            "n_sensors": M, "latent_dim": P, "width": W, "depth": D, "n_fourier": NFF, "lr": LR,
            "iterations": iters, "batch": BATCH, "points": PT, "seeds": seeds, "mode": mode,
    ...
    torch.save(best_solver.model.net.state_dict(), os.path.join(OUT, "deeponet_net.pt"))
    np.savez(os.path.join(OUT, "deeponet_meta.npz"), sidx=best_solver.sidx,
             Tmax=best_solver.Tmax, m=M, nff=NFF, t_train_end=te)
...
if __name__ == "__main__":
    m = "smoke" if "--smoke" in sys.argv else ("quick" if "--quick" in sys.argv else "full")
    main(mode=m)
